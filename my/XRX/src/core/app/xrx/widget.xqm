...
declare function widget:key($atomid as xs:string) as xs:string {
    tokenize($atomid, '/')[last()]
};
declare function widget:mainwidgetid($atomid as xs:string) as xs:string {
    replace($atomid, '/widget/', '/mainwidget/')
};
declare function widget:embeddedwidgetid($atomid as xs:string) as xs:string {
    replace($atomid, '/widget/', '/embeddedwidget/')
};
...
declare function widget:constructor-expression($constructor as element(xrx:constructor)*, $pass as element(xrx:pass)*) {
    ...
    let $expression := if ($passed-expression != '') then
        $passed-expression
    else
        $default-expression
    return
    ...
    if ($instances) then
        <div
            ...
            xmlns="http://www.w3.org/1999/xhtml">
            {
                ...
                return
                    <div
                        class="xrx-instance">{$instance/@*}{$instance/text()}</div>
            }
        </div>
    else
        ()
...
$mode as xs:string) as element() {
    ...
    if (($flag = 'true' and xmldb:get-current-user() != 'guest') or matches($widgetkey, '(registration|request-password|iipmooviewer)')) then
        ...
        false()
    let $jqueryflag := if ($widget/xrx:init/xrx:processor/xrx:jqueryflag/text() = 'true') then
        true()
    else
        false()
    let $bootstrapflag := if ($widget/xrx:init/xrx:processor/xrx:bootstrapflag/text() = 'true') then
        true()
    else
        false()
    let $javascript-debug := if ($widget/xrx:jss/@debug/string() = 'true') then
        true()
    else
        false()
        ...
        if ($mode = 'mainwidget') then
            if ($xforms-flag) then
                <html
                        ...
                        <title>{{ if($xrx:mainwidget/xrx:title/xrx:i18n) then i18n:translate($xrx:mainwidget/xrx:title/xrx:i18n) else() }}{{ if($xrx:htdoc//xrx:i18n) then i18n:translate($xrx:htdoc//xrx:i18n) else() }}&#160;{{ string-join(subsequence($xrx:tokenized-uri, 1, count($xrx:tokenized-uri) - 1), '|') }}&#160;-&#160;{{ conf:param("platform-browser-title") }}</title>
                        {
                            if ($bootstrapflag) then
                                ...
                                <link
                                    rel="stylesheet"
                                    type="text/css"
                                    href="http://glossa.uni-graz.at/gamsdev/gschneider/mom/css/main.css"/>,
                                <link
                                ...
                                if ($jqueryflag) then
                                    <link
                                        rel="stylesheet"
                                        href="/{$project-name}/jquery/themes/base/jquery.ui.all.css"/>
                                else
                                    (),
                                <link
                                    rel="stylesheet"
                                    type="text/css"
                                    href="/{$project-name}/css/?atomid={$widgetid}"/>
                                )
                        }
                        {
                            if (not($javascript-debug)) then
                                if ($widget/xrx:jss/xrx:resource or $jqueryflag) then
                                    ...
                                    <script
                                        ...
                                        src="/{$project-name}/javascript/?atomid={$widgetid}"/>,
                                ...
                                return
                                    ...
                                    <script
                                        type="text/javascript"
                                        src="/{$project-name}/resource/?atomid={$id}"/>,
                                    $widget/xrx:jss/xhtml:script,
                                    $portal/xrx:jss/xhtml:script
                                    )
                        }
                    ...
                    <body
                        data-xrx-widget="{$portalid}">
                        {{
                        if(auth:matches(<xrx:rule
                            xmlns:xrx="http://www.monasterium.net/NS/xrx"><xrx:user/><xrx:role>translator</xrx:role></xrx:rule>)) then
                        <div
                            class="xrx-translate-page-link">
                            <a
                                href="/{$project-name}/translate?id={$widgetid}">‣&#160;{{ i18n:translate(<xrx:i18n><xrx:key>translate-page</xrx:key><xrx:default>Translate page</xrx:default></xrx:i18n>) }}</a>
                        </div>
                        else()
                        }}
                        {
                            if ($portal) then
                                widget:preprocess($portal, $widget, $xrx-live-project-db-base-collection)
                            else
                                ()
                        }
                        <div
                            class="xrx-data-messages"
                            style="display:none">{{ i18n:as-json(xrx:messages($xrx:mainwidget)) }}</div>
                    </body>
                    )
                    }}
                </html>
        ...
        element {node-name($element)} {
            ...
            return
                typeswitch ($child)
                    ...
                    case element(xrx:subwidget)
                        return
                            
                            let $subwidgetid := if ($child/xrx:atomid) then
                                $child/xrx:atomid/text()
                            else
                                $child/text()
                                ...
                                if ($subwidget) then
                                    <div
                                        ...
                                        class="xrx-subwidget">
                                        {
                                            (
                                            <div
                                                style="display:none">{$subwidget/xrx:model/node()}</div>,
                                            '{ ',
                                            ...
                                            <div
                                                class="xrx-widget"
                                                xmlns="http://www.w3.org/1999/xhtml"
                                                data-xrx-widget="{$subwidgetid}">{widget:preprocess($subwidget, $main-widget, $base-collection)}</div>,
                                            '}'
                                            )
                                        }
                                    </div>
                    ...
                    case element(xrx:mainwidget)
                        return
                            ...
                            if ($main-widget) then
                                ...
                                <div
                                    style="display:none"
                                    xmlns="http://www.w3.org/1999/xhtml">{$main-widget/xrx:model/node()}</div>,
                                <div
                                    class="xrx-widget"
                                    xmlns="http://www.w3.org/1999/xhtml"
                                    data-xrx-widget="{$main-widget/xrx:id/text()}">{
                                        widget:preprocess($main-widget, <span
                                            xmlns="http://www.w3.org/1999/xhtml"/>, $base-collection)
                                    }</div>
                                )
                            else
                                ()
                    
                    case element(xrx:i18n)
                        return
                    ...
                    case element(xrx:attributes)
                        return
                            
                            <div
                                class="xrx-attributes"
                                xmlns="http://www.w3.org/1999/xhtml"/>
                    
                    case element(xrx:resource)
                        return
                            ...
                            if (starts-with($child/@type, 'image')) then
                                ...
                                let $binary := if (util:binary-doc-available($uri)) then
                                    util:binary-doc($uri)
                                else
                                    ()
                                return
                                    <img
                                        src="{concat('data:', $child/@type/string(), ';base64,', $binary)}"
                                        xmlns="http://www.w3.org/1999/xhtml">{$child/@*[name(.) != 'type']}</img>
                            
                            else
                                ()
                    
                    case element(xrx:div)
                        return
                            
                            let $key := $child/text()
                            let $div := root($child)//xrx:divs/xrx:div[xrx:key = $key]
                            return
                                if ($div/xrx:view) then
                                    widget:preprocess($div, (), $base-collection)
                                else
                                    ()
                    
                    
                    case element(xrx:layout)
                        return
                            ...
                            return
                                <div
                                    ...
                                    xmlns="http://www.w3.org/1999/xhtml">
                                    {
                                        ...
                                        return
                                            typeswitch ($e)
                                                case element(xrx:options)
                                                    return
                                                        ()
                                                case element(xrx:center)
                                                    return
                                                        widget:parse(<div
                                                            class="ui-layout-center">{$e/*}</div>, $main-widget, $base-collection)
                                                case element(xrx:north)
                                                    return
                                                        widget:parse(<div
                                                            class="ui-layout-north">{$e/*}</div>, $main-widget, $base-collection)
                                                case element(xrx:east)
                                                    return
                                                        widget:parse(<div
                                                            class="ui-layout-east">{$e/*}</div>, $main-widget, $base-collection)
                                                case element(xrx:south)
                                                    return
                                                        widget:parse(<div
                                                            class="ui-layout-south">{$e/*}</div>, $main-widget, $base-collection)
                                                case element(xrx:west)
                                                    return
                                                        widget:parse(<div
                                                            class="ui-layout-west">{$e/*}</div>, $main-widget, $base-collection)
                                                default
                                                    return
                                                        widget:parse($e, $main-widget, $base-collection)
                                    }
                                </div>
                    
                    case element(xrx:repeat)
                        return
                            ...
                            <div
                                class="xrx-repeat"
                                xmlns="http://www.w3.org/1999/xhtml"
                                data-xrx-bind="{$child/@bind/string()}">{$child/*}</div>,
                            $main-widget,
                            $base-collection
                            )
                    case element(xrx:select)
                        return
                            ...
                            <div
                                ...
                                xmlns="http://www.w3.org/1999/xhtml">
                                {
                                    ...
                                    return
                                        ...
                                        widget:parse(<label
                                            for="{$input-id}">{$item/xrx:label/node()}</label>, $main-widget, $base-collection)
                                        )
                                }
                            </div>
                    
                    case element(xrx:switch)
                        return
                    ...
                    case element(xrx:visualxml)
                        return
                            ...
                            <textarea
                                ...
                                xmlns="http://www.w3.org/1999/xhtml">
                                {
                                    if (exists($child/@attributes)) then
                                        attribute data-xrx-attributes {$child/@attributes/string()}
                                    else
                                        ()
                                }
                                {
                                    if (exists($child/@elements)) then
                                        attribute data-xrx-elements {$child/@elements/string()}
                                    else
                                        ()
                                }
                                {
